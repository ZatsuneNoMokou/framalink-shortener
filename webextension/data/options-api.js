...
async function getPreference(prefId) {
	...
	let currentPreferences = await browser.storage.local.get(prefId);
...
/**
 *
 * @param {boolean|number|string} string
 * @return {boolean}
 */
...
function translateNodes(locale_document) {
	...
	let translate_nodes = locale_document.querySelectorAll('[data-translate-id]');
	for(let i in translate_nodes) {
		if (translate_nodes.hasOwnProperty(i) === false) continue;
...
function translateNodes_title(locale_document) {
	let _ = chrome.i18n.getMessage;
	let translate_nodes = locale_document.querySelectorAll('[data-translate-title]');
	for(let i in translate_nodes) {
		if (translate_nodes.hasOwnProperty(i) === false) continue;
